...
The data provided for this project is generated by a graph-based multi-agent reinforcement learning problem Daniel is working on for his Masters Degree.  This visualization helps guide the user in seeing the value for each team and player resources and rewards.  The scenarios of the simulation can be changed.  These changes can be the number of edges players travel, the number of players, placement of items, and the type and function of rewards.  Converted to json, the data is a simulation Object.  This Object gives the relative placement of edges and the relative placement of players and items.  We want to give a definite placement where a user can see players and items move on paths and how the attributes of each team and player change.

## Related Work

The final implementation of this visualization is a node link diagram.  Unlike other projects with node link diagrams this implementation does not contain data for the nodes.  The nodes only act as a visual representation of where edges connect.  All of the data is contained on the edges.  This understanding led us to the use of matrix table to represent the data for each node.  Matrix diagrams are also a good way to show heat diagrams as shown in class.

For the node link diagram it became evident the graph would be very cluttered and see individual paths would be difficult.  We then were shown the use of force directed functions that could help space the nodes and edges of the graph.  This function uses the position of nodes and then moves to create the optimal view for the graph.

## Questions

The main question for this visualization is what does the simulation look like.  Daniel and his team need to see how different movements and reward functions cause different reactions or strategies.  Once Daniel and his team begin changing the reward functions they need to be able to see how their changes change what occured.  They also would like to see placement of items relative to the players.  During the implementation of the visualization we began to see that items placed in the simulation were not in expected places.  These are things users should be able to see.
...
From the implementation of our previous building phase we had a node link diagram that was not spaced evenly.  This caused a lot of clutter and overlap for the edges.  During our class presentation we felt that new viewers were confused and struggled to understand what the node link diagram represented.  This caused us to focus on building the map with a matrix representation or using a force diagram method.  We found with the matrix representation that paths would be overlayed on top of each other hiding path data.  This design did not cope with the playing field layout.  This made using a force directed method even more important.  

## Design Evolution

The design of this visualization changed frequently throughout this phase.  We began looking at a matrix representation of the map links. This allowed the user to see more information in the form of text for each link.  The matrix was also simpler to implement compared to the force directed node link diagram.  However this design current does not account for multiple links starting and ending at similar nodes.  The links began to overlap and hide other links.

The node link diagram evolved with the force directed function.  This function allowed the links in the diagram to be more visible and have less overlap.  The overlap cluttered the map and caused the user to be unable to see specific and useful links.  Because the node link diagram does not allow data to be easily displayed on the links we added graphs to the visualization to see the specific placement of players and items.

## Implementation

The key interactivity of the visualization is the ability for users to change the time of the simulation.  This allows the user to see the evolution of the data as the simulation progressed.

<img src="proposal/simTime.jpg">

In the overview table the user has the ability to view different attributes in the data and change which team the user would like to analyze using the buttons in the table.

<img src="proposal/finalTable.jpg">

Towards the end of the building phase we implemented the ability to add a graph containing data for selected map links.  This occurs when the user clicks on a link in the node link diagram.  Once clicked the link graph is mapped out with the significant players and items pertaining to the path.

<img src="proposal/pathGraphs.jpg">

## Evaluation

During the creation this visualization we found data from the simulation was completely wrong.  Players and items wondered out of bounds for different paths.  We also learned that the teams are not evenly balanced for this simulation.  One team overpowered the other team when it came to displaying the visualization data.

To see the simulation play out we implemented the ability to change the time of simulation.  Not only does this function work for the node link diagram, but it also works for the link graphs.  The user can watch as players and items move about the playing field.  The data is also shown on the node link diagram to help the user understand what attributes are contained at each path.  The comparison graphs allow the user to see a condensed view of the team statistics over the entire simulation.

The design can be improved by utilizing the white space of the visualization.  The node link diagram takes a lot of space.  For future improvements it could potential be condensed and better positioned.  Further improvements could also include positioning players and items on the node link diagram, aswell as highlighting edges in the node link diagram with their corresponding link graphs.  Tooltips can be implemented to view overall data for an edge or for a specific player or item.

A personal improvement Daniel hopes to include is being able to see the simulation progress over time in a film like manner.
